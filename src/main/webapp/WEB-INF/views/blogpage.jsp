...
<h1class="page-header">${blog.blogTitle}</h1>

<p><small>-given by </small> <i>${blog.blogname}</i> on ${blog.creationDate}</p>

<p> ${blog.blogContent}</p>

<%-- <a class="btn btn-primary" href="deleteBlog?id=${blog.blogId}" role="button" >Delete</a></td>
			 --%>
<form:form action="./addComment?id=${blog.blogId}"
									commandName="cmt" method="post">
									<form:textarea placeholder="comment" class="form-control input-md" path="cmt"></form:textarea>
											...
											<div class="message">
												...
												<p>
													<strong>${a.userid},<em> posted at
															${a.creationDate}</em></strong> ${a.cmt}
											</div>
										</div>
									</c:forEach>
